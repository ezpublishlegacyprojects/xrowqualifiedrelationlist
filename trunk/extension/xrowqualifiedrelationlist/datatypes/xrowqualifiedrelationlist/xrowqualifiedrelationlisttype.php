...
class xrowQualifiedRelationlistType extends eZDataType
{
    const DATA_TYPE_STRING = "xrowqualifiedrelationlist";

    /*!
     Initializes with a string id and a description.
    */
    function xrowQualifiedRelationlistType()
    {
    ...
    */
    function fetchObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        $contentClassAttribute = $contentObjectAttribute->contentClassAttribute();
        $classContent = $contentClassAttribute->content();
        $qualifierPostVariableName = $base . "_qualifier_" . $contentObjectAttribute->attribute( "id" );
        $selectedQualifierIDArray = $http->hasPostVariable( $qualifierPostVariableName ) ? $http->postVariable( $qualifierPostVariableName ) : false;
        ...
        if ( $classContent['selection_type'] != 0 )
        {
            ...
            foreach ( $selectedObjectIDArray as $key => $objectID )
            {
                ...
                ++$priority;
                $content['relation_list'][] = $this->appendObject( $objectID, $priority, $selectedQualifierIDArray[$key], $contentObjectAttribute );
            }
        ...
        for ( $i = 0; $i < count( $content['relation_list'] ); ++$i )
        {
        	$content['relation_list'][$i]['qualifier'] = $selectedQualifierIDArray[$i];
    ...
    function storeClassAttributeContent( $classAttribute, $content )
    {
        if ( is_array( $content ) )
        {
            $doc = xrowQualifiedRelationlistType::createClassDOMDocument( $content );
            xrowQualifiedRelationlistType::storeClassDOMDocument( $doc, $classAttribute );
            return true;
        }
        return false;
    }
    ...
    static function storeObjectAttributeContent( $objectAttribute, $content )
    {
        if ( is_array( $content ) )
        {
            $doc = xrowQualifiedRelationlistType::createObjectDOMDocument( $content );
            xrowQualifiedRelationlistType::storeObjectDOMDocument( $doc, $objectAttribute );
            return true;
        }
        return false;
    }
    ...
    */
    ...
    function createObjectDOMDocument( $content )
    {
        ...
        $attributeDefinitions = xrowQualifiedRelationlistType::contentObjectArrayXMLMap();
    ...
    function deleteStoredObjectAttribute( $objectAttribute, $version = null )
    {
             ...
             is_array( $content['relation_list'] ) )
        {
            ...
            {
                xrowQualifiedRelationlistType::removeRelationObject( $objectAttribute, $deletionItem );
            }
            $db->commit();
        }
    }
    ...
    function customObjectAttributeHTTPAction( $http, $action, $contentObjectAttribute, $parameters )
    {
             ...
             $action == 'new_class' )
        {
            ...
            if ( $class )
            {
                ...
                $relationItem = xrowQualifiedRelationlistType::createInstance( $class,
                                                                          $priority + 1,
                ...
                eZDebug::writeError( "Unknown class ID $classID, cannot instantiate object",
                                     'xrowQualifiedRelationlistType::customObjectAttributeHTTPAction' );
        }
                  ...
                  $action == 'remove_objects' )
        {
            ...
            else if ( $action == 'remove_objects' )
            {
                ...
                foreach( $relationList as $relationItem )
                {
                    ...
                    {
                        xrowQualifiedRelationlistType::removeRelationObject( $contentObjectAttribute, $relationItem );
                    }
                    else
                    {
    ...
    */
    ...
    {
        if ( xrowQualifiedRelationlistType::isItemPublished( $deletionItem ) )
        {
            return;
        }
        ...
        if ( $isDeletionAllowed )
        {
            ...
            else
            {
                eZDebug::writeError( 'Cleanup of subobject-version failed. Could not fetch object from relation list.\n' .
                                     ...
                                     'xrowQualifiedRelationlistType::removeRelationObject' );
            }
        }
    }
    ...
    function createInstance( $class, $priority, $contentObjectAttribute, $nodePlacement = false )
    {
        ...
        $relationItem = array( 'identifier' => false,
                               'priority' => $priority,
                               'qualifier' => eZINI::instance( 'xrowqualifiedrelationlist.ini' )->variable( 'Settings', 'Default' ), 
                               'in_trash' => false,
    ...
    function appendObject( $objectID, $priority, $qualifier, $contentObjectAttribute )
    {
    ...
    function fixRelatedObjectItem ( $contentObjectAttribute, $objectID, $mode )
    {
        switch ( $mode )
        {
            case 'move':
            {
                xrowQualifiedRelationlistType::fixRelationsMove( $objectID, $contentObjectAttribute );
            } break;

            case 'trash':
            {
                xrowQualifiedRelationlistType::fixRelationsTrash( $objectID, $contentObjectAttribute );
            } break;

            case 'restore':
            {
                xrowQualifiedRelationlistType::fixRelationsRestore( $objectID, $contentObjectAttribute );
            } break;

            case 'remove':
            {
                xrowQualifiedRelationlistType::fixRelationsRemove( $objectID, $contentObjectAttribute );
            } break;

            case 'swap':
            {
                xrowQualifiedRelationlistType::fixRelationsSwap( $objectID, $contentObjectAttribute );
            } break;

            default:
            {
                eZDebug::writeWarning( $mode, 'Unknown mode xrowQualifiedRelationlistType::fixRelatedObjectItem()' );
            } break;
        }
    }
    ...
    */
    function objectAttributeContent( $contentObjectAttribute )
    {
        ...
        if ( trim( $xmlText ) == '' )
        {
            $objectAttributeContent = xrowQualifiedRelationlistType::defaultObjectAttributeContent();
            return $objectAttributeContent;
        }
        $doc = xrowQualifiedRelationlistType::parseXML( $xmlText );
        $content = xrowQualifiedRelationlistType::createObjectContentStructure( $doc );
    ...
    function classAttributeContent( $classAttribute )
    {
        ...
        if ( trim( $xmlText ) == '' )
        {
            return xrowQualifiedRelationlistType::defaultClassAttributeContent();
        }
        $doc = xrowQualifiedRelationlistType::parseXML( $xmlText );
        return xrowQualifiedRelationlistType::createClassContentStructure( $doc );
    }
    ...
    {
        $content = xrowQualifiedRelationlistType::defaultObjectAttributeContent();
        $root = $doc->documentElement;
        $relationList = $root->getElementsByTagName( 'relation-list' )->item( 0 );
        if ( $relationList )
        {
            $contentObjectArrayXMLMap = xrowQualifiedRelationlistType::contentObjectArrayXMLMap();
        ...
        for ( $i = $relationItems->length - 1; $i >= 0; $i-- )
        {
            ...
            if ( $object === null )
            {
                eZDebug::writeWarning( "Object with remote id '$relatedObjectRemoteID' not found: removing the link.",
                                       'xrowQualifiedRelationlistType::unserializeContentObjectAttribute()' );
    ...
    */
    function removeRelatedObjectItem( $contentObjectAttribute, $objectID )
    {
        ...
        xrowQualifiedRelationlistType::storeObjectDOMDocument( $doc, $contentObjectAttribute );
        return $return;
    }
...
eZDataType::register( xrowQualifiedRelationlistType::DATA_TYPE_STRING, "xrowQualifiedRelationlistType" );
